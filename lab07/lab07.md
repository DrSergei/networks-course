...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Sequence number равен 0.
   - У него выставлен флаг SYN.
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Sequence number равен 0.
   - Acknowledgement number равен 1.
   - Т.к. это происходило во время трехэтапного рукопожатия, то он просто увеличил на единичку Sequence number пришедшего пакета.
   - Выставлен флаг SYN и ACK.
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - Sequence number равны 1, 729, 1300, 2029, 3329, 4629.
   - 3.103822 для первого и  3.104044 для остальных.
   - Они были получены для некоторых сразу во время 3.3009333.
   - Для первого оно равно (3.3009333 - 3.103822)=0.1971с а для остальных (3.3009333 - 3.104044)=0.1968с.
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Можно оценить его с помощью тех первых пакетов, передача данных началась в 3.104044 причем можно заметить, что почти всегда пакетами одного размера (1354) и до времени 3.3009333 было передано 12821-729=12092 байт данных без учета запроса httt.
   - Тогда в среднем это было 12092/0.1968=61.44Кб/c.
...
$L = 1500$ байт, $U=\dfrac{nL/R}{RTT + L/R}$ по формуле с 6 лекции. Тогда нам надо решить неравенство $\dfrac{nL/R}{RTT + L/R} > 0.98$. Подставим числа $\dfrac{L}{R}=\dfrac{1500*8}{10^9}=0.000012$c, то есть 0.012мс. Тогда получим формулу $U=\dfrac{n*0.012}{30+0.012}>0.98$ и получаем т.к. $n$ натуральное, что $n \geq 2451$.
