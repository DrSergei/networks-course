...
3. Каково значение исходного Ethernet-адреса? Какое устройство имеет такой Ethernet-адрес?
   - 26:40:cd:b6:e0:75
   - Адрес маршрутизатора т.к. сервер в другой сети
...
#### Решение
1. Вероятность что конкретный узел начнет передачу равна $p$, а все остальные должны не начать $(1-p)^{N-1}$, а т.к. узлов всего $N$, то $Np(1-p)^{N-1}$ вероятность что получится начать передачу на текущем кванте. Тогда пусть $q=Np(1-p)^{N-1}$, тогда матожидание равно $\sum_iiq(1-q)^{i-1}=\dfrac{1-q}{q}$ т.к. это просто геометрическое распределение. Тогда $x=\dfrac{1-Np(1-p)^{N-1}}{Np(1-p)^{N-1}}$. А эффективность равна $\dfrac{k}{k + \frac{1-Np(1-p)^{N-1}}{Np(1-p)^{N-1}}}$.
2. Нужно найти минимум $x$, это равносильно максимуму $Np(1-p)^{N-1}$. Для этого приравняем производную нулю как в прошлой лабораторной и получим $p=\dfrac{1}{N}$.
3. Посчитаем предел $x$, для этого посчитаем предел $Np(1-p)^{N-1}$, аналогично прошлой лаборатной получим $\dfrac{1}{e}$. Тогда $x=e-1$, а тогда эффективность равна $\dfrac{k}{k+e-1}$.
4. Тогда увеличивая $k$ и за ним $L$, мы видим что эффективность стремится к 1 т.к. числитель и знаменатель дроби отличаются на константу.
...
#### Решение
1. Скорость записи равна $128*1000/8=16000$ байт/сек. Тогда задержка равна $\dfrac{L+5}{16000} * 1000=\dfrac{L+5}{16}$ мс.
2. Для ethernet она равна $\dfrac{1500+5}{16} \approx 94$ мс, а для ATM $\dfrac{50+5}{16} \approx 3.5$ мс.
3. Скорость равна $622 * 1000000 / 8 =77750000$ байт в секунду, тогда ответ равен $\dfrac{1500+5}{77750000} * 1000 \approx 0.02$ мс и $\dfrac{50+5}{77750000} * 1000 \approx 0.001$ мс.
4. Меньше вероятность ошибки в одном пакете и лучшая возможность коррекции ошибок. Vеньше нужно служебных бит для восстановление или меньше времени на повторную пересылку, что повышает отзывчивость.
